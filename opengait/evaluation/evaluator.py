...
def evgait_evaluator(data, dataset, metric='euc'):
    ...
    for t, p, l in zip(seq_type, pred, label):
        type_total[t] += 1
        if p == l:
            type_correct[t] += 1
    for t in type_total:
        type_acc = type_correct[t] / type_total[t]
        result_dict[f'scalar/test_accuracy/{t}'] = type_acc
        get_msg_mgr().log_info(f'Type: {t}, Accuracy: {type_acc*100:.2f}%')
    # overall
    result_dict['scalar/test_accuracy/overall'] = accuracy
    get_msg_mgr().log_info(f'Overall Accuracy: {accuracy*100:.2f}%')

def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    probe_seq_dict = {'CASIA-B': {'NM': ['nm-05', 'nm-06']},            # 查询
                    ...
                    #   'EV-CASIA-B': ['nm'],
                      ...
                      'DVS128' : {'NM' : [f'nm-{i}' for i in range(1, 30)]}        # DVS128
                      }
    gallery_seq_dict = {'CASIA-B': ['nm-01', 'nm-02', 'nm-03', 'nm-04'],        # 注册
                        # 'CASIA-B': ['nm-01', 'nm-02', 'nm-03', 'nm-04'],
                        'OUMVLP': ['01'],
                        'CASIA-E': ['H-scene1-nm-1', 'H-scene1-nm-2', 'L-scene1-nm-1', 'L-scene1-nm-2'],
                        'SUSTech1K': ['00-nm'],
                        'DVS128' : [f'nm-{i}' for i in range(30, 55)]        # DVS128
                        }
    msg_mgr = get_msg_mgr()
    acc = {}        # 存储各条件的准确率矩阵
    view_list = sorted(np.unique(view))
...
def evaluate_CCGR_MINI(data, dataset, metric='euc'):
    ...
    gallery_sets = json.load(
        open('./datasets/CCGR-MINI/CCGR-MINI_Lite.json', 'rb'))['GALLERY_SET']
    probe_mask = []
    for id, ty, sq in zip(labels, cams, time_seqs):
        id = str(int(id) - 1057)
        sq = sq + '.avi'
