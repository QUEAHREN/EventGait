...
class HorizontalPoolingPyramid():
    """
    HPP会将特征图在宽度方向分割成不同的水平条带，类似金字塔，最终p=sum(bin_num)=31 输出为[n,c,p=31]
    ...
    def __call__(self, x):
        ...
        for b in self.bin_num:
            ...
            features.append(z)
        # print('2 ', torch.cat(features, -1).shape)
...
class SetBlockWrapper(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if isinstance(x, tuple):
            ...
            heat_size = x[1].size()
            return x[0].reshape(n, s, *output_size[1:]).transpose(1, 2).contiguous(), x[1].reshape(n, s, *heat_size[1:]).transpose(1, 2).contiguous()
        else:
            output_size = x.size()
...
class SeparateFCs(nn.Module):
    # 对输入的 ​​p 个部件特征​​（如水平金字塔生成的31个条带）​​分别应用独立的全连接层​​，每个部件有自己专属的权重矩阵
