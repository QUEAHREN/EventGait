...
## SimpleNet + dynamic branch + feature fusion + dino
...
class MatchingUnit(nn.Module):
    def __init__(self, chunk_size=6):
        ...
        self.dynamic_channel_conv = nn.Conv2d(128*chunk_size, 128, kernel_size=1, stride=1, padding=0)
...
class EventGait(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        # 特征融合
        self.matching = MatchingUnit(chunk_size=model_cfg['chunk_size'])
        ...
        self.student_trans = nn.Conv2d(128, 1536, kernel_size=1, stride=1, padding=0, bias=False)
        self.student_trans = SetBlockWrapper(self.student_trans)


    def forward(self, inputs):
        ipts, labs, _, _, seqL = inputs
        frames_ipts = ipts[0]      # torch.Size([Batch, 30, 3, 128, 128]) n s c h w
        events_ipts = ipts[1]        # torch.Size([Batch, 180, 3, 128, 128]) n s c h w
        if self.training:
            rgbs_ipts = ipts[2]      # torch.Size([Batch, 30, 3, 128, 128]) n s c h w
        if len(frames_ipts.size()) == 4:
            frames_ipts = frames_ipts.unsqueeze(1)
        frames_ipts = rearrange(frames_ipts, 'n s c h w -> n c s h w')  # torch.Size([4, 3, 30, 128, 128])
        del ipts

        static_feat = self.static_branch(frames_ipts)       # static_feat = torch.Size([4, 128, 30, 16, 16])
