...
class HWC2CHW(object):
    ...
    def __call__(self, seq):
        if len(seq.shape) == 5:     # s, chunk, h, w, c
            s, chunk, h, w, c = seq.shape
            seq = np.transpose(seq, (0, 1, 4, 2, 3))
        elif len(seq.shape) == 4:   # s, h, w, c
            s, h, w, c = seq.shape
            seq = np.transpose(seq, (0, 3, 1, 2))
        else:
            raise ValueError("Invalid input shape for HWC2CHW.{}".format(seq.shape))
        return seq
    
class NormalizeCHW(object):
...
class EventsNoise(object):
    # COPY from :https://github.com/AHupuJR/EFNet/blob/main/scripts/data_preparation/noise_function.py
    def __init__(self, noise_std=1.0, noise_fraction=0.05, alpha=1.0):
        self.noise_std = noise_std
        self.noise_fraction = noise_fraction
        self.alpha = alpha

    # you should use it after HWC2CHW
    def put_hot_pixels_in_voxel_(self, voxel, hot_pixel_range=20, hot_pixel_fraction=0.00002):
        height, width = voxel.shape[-2:]
        num_hot_pixels = int(hot_pixel_fraction * width * height)
        if num_hot_pixels == 0:
            return voxel
        x = np.random.randint(0, width, size=num_hot_pixels)
        y = np.random.randint(0, height, size=num_hot_pixels)
        v = np.random.randint(-hot_pixel_range, hot_pixel_range, size=num_hot_pixels)
        for i in range(num_hot_pixels):
            voxel[..., :, y[i], x[i]] = v[i]
        return voxel


    def add_noise_to_voxel(self, voxel, noise_std=1.0, noise_fraction=0.05, alpha=1.0):
        noise = np.random.normal(loc=0.0, scale=noise_std, size=voxel.shape)  # mean = 0, std = noise_std
        noise[1, :, :] = 0  # green channel has no noise
        if noise_fraction < 1.0:
            mask = np.random.random_sample(voxel.shape) >= noise_fraction
            noise[mask] = 0
        return voxel + noise * alpha


    def __call__(self, x):
        x = self.add_noise_to_voxel(x, noise_std=self.noise_std, noise_fraction=self.noise_fraction, alpha=self.alpha)
        # x = self.put_hot_pixels_in_voxel_(x, hot_pixel_range=20, hot_pixel_fraction=0.00002)
        return x
    
class MergeST(object):
    def __init__(self):
        pass

    def __call__(self, x):
        s, chunk, _1, _2, _3 = x.shape
        return x.reshape(-1, _1, _2, _3)

class ReshapeHWC(object):
    def __init__(self, new_h=64, new_w=64):
        self.H = new_h
        self.W = new_w

    def __call__(self, x):
        T, H, W, C = x.shape
        resized = np.zeros((T, self.H, self.W, C), dtype=x.dtype)
        for t in range(T):
            resized[t] = cv2.resize(x[t], (self.W, self.H), interpolation=cv2.INTER_LINEAR)
        return resized


class RatioCrop(object):
    def __init__(self, ratio=2.0):
        self.ratio = ratio  # height / width
    ...
    def __call__(self, x):
        ...
        if current_ratio > self.ratio:
            ...
            start_h = (H - new_H) // 2
            x = x[:, start_h:start_h + new_H, :, :]
        elif current_ratio < self.ratio:
            # need to crop width
            new_W = int(H / self.ratio)
            start_w = (W - new_W) // 2
            x = x[:, :, start_w:start_w + new_W, :]
        return x

# only for cross-illumination task
