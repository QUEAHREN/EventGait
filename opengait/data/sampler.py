...
class TripletSampler(tordata.sampler.Sampler):
    def __init__(self, dataset, batch_size, batch_shuffle=False):
        ...
        self.world_size = dist.get_world_size()     # 这个是进程数，是否就是GPU的个数？
    ...
    def __iter__(self):
        while True:
            ...
            pid_list = sync_random_sample_list(
                self.dataset.label_set, self.batch_size[0])     # 表示每个batch中包含多少个不同ID，返回值的len=self.batch_size[0]

            for pid in pid_list:
                indices = self.dataset.indices_dict[pid]        # 每个label的索引映射
                # 按分析，len(indices) = 该id的type个数*该id每个type的view个数
                # print(len(indices)) 结果确实=11
                indices = sync_random_sample_list(
                    indices, k=self.batch_size[1])              # 表示对于这些ID的每一个，要随机采样多少个步态序列, 返回值的len=self.batch_size[1]
            ...
            yield sample_indices        # 返回的是一个索引列表迭代器
